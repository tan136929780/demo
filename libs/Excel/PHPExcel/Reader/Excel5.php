...
//			var_dump($bcBitCount);
	...
	private function _getFormulaFromData($formulaData,  $additionalData = '', $baseCell = 'A1')
	{
		...
		while (strlen($formulaData) > 0 && $token = $this->_getNextToken($formulaData, $baseCell)) {
			$tokens[] = $token;
		...
		if (isset($this->_ref[$index])) {
			...
			switch ($type) {
				case 'internal':
					...
					if ($this->_ref[$index]['firstSheetIndex'] == 0xFFFF || $this->_ref[$index]['lastSheetIndex'] == 0xFFFF) {
	...
	private static function _mapBuiltInColor($color)
	{
		switch ($color) {
			...
			case 0x41: // system window background color
